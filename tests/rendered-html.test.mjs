...
test("server-renders the ClinoraMedBill homepage", async () => {
  ...
  assert.match(html, /Talk to an expert/i);
  assert.match(html, /A Trusted Medical Billing Partner for Healthcare Providers/i);
  assert.match(html, /Complete Healthcare Revenue Cycle Solutions/i);
  assert.match(html, /Billing Solutions for Every Type of Healthcare Sector/i);
  assert.match(html, /Medical Billing Services for Every Healthcare Specialty/i);
  assert.match(html, /Supporting Healthcare Providers Across the Nation/i);
  assert.match(html, /Billing Support That Works With Your Existing EHR\/EMR/i);
  assert.match(html, /Insights to Help You Manage Your Practice Better/i);
  assert.match(html, /Ready to reduce denials and get paid faster/i);
  assert.match(html, /Questions providers ask before partnering/i);
  assert.match(html, /usa-states-map\.svg/i);
  assert.match(html, /commitment-specialties\.svg/i);
  assert.match(html, /commitment-hipaa\.svg/i);
  assert.match(html, /commitment-satisfaction\.svg/i);
  ...
  assert.match(html, /Wherever your practice is located/i);
  ...
  assert.match(html, /EHR\/EMR Softwares/i);
  assert.match(html, /name="ehr"/i);
  assert.doesNotMatch(html, /class="partner-intro-status"/i);
  assert.doesNotMatch(html, /class="home-audience-photo home-audience-photo-secondary"/i);
  assert.doesNotMatch(html, /class="specialty-number"/i);
  assert.doesNotMatch(html, /class="blog-card-count"|class="blog-card-category"/i);
  assert.doesNotMatch(html, /home-ehr-carousel-instructions/i);
  assert.doesNotMatch(html, /Free audit request/i);
...
test("routes enquiry forms through the validated email endpoint", async () => {
  ...
  assert.match(home, /submitAudit\(event, "ehr-consultation"\)/);
  ...
  assert.match(endpoint, /"ehr-consultation"/);
